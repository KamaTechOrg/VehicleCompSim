...
#define LISTENING_PORT 8102           // This device port
#define BUFLEN 1024


#define BASE_TEMP 25   // Base temperature
#define TEMP_VARIATION 2 // Maximum variation in temperature
#define SENSOR_NAME "Temperature Sensor" // Define the sensor name
...
void run_server() {
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            // Now, keep sending temperature readings every 2 seconds
            while (1) {
                ...
                char temp_str[128];
                sprintf(temp_str,"ID:%s,VALUE:%d", SENSOR_NAME, current_temp);

                // Send the temperature to the client
                err = send(sock, temp_str, strlen(temp_str), 0);
                if (err < 0) {
                    ESP_LOGE(TAG, "Failed to send temperature: errno %d", errno);
                    break;
                }
                ESP_LOGI(TAG, "Sent temperature to client: %s", temp_str);

                sleep(2);  // Wait for 2 seconds before sending the next temperature
            }
        }
