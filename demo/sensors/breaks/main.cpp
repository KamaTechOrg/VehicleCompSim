...
int main()
{
    ...
    std::cout << "breaks starting";

    ClientSocket client(vehicle_components_id_config::BREAKS);
    ...
    {
        amount = rand() % 10 + 1;
        m =  to_string(ACTIONS::PRESS_BREAKS) + " " + to_string(amount);

        sendErrorCode code = client.send((void *)m.c_str(), m.size(), vehicle_components_id_config::BREAKS, d_id);
