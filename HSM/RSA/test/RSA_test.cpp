...
TEST(RSA_BigNum_test, modInverse_test_1)
{
    ...
    BigNum e("10001");
    BigNum phi = (p - 1) * (q - 1);
    EXPECT_EQ(RSA::modInverse(e, phi), d);
}
...
TEST(RSA_BigNum_test, encryption_and_decryption_tst_for_int)
{
    ...
    BigNum encrypted = RSA::power(message, e, p * q);
    BigNum decrypted = RSA::decrypt(encrypted, d, p * q);
...
TEST(RSA_BigNum_test, encryption_and_decryption_tst_for_int_2)
{
    ...
    BigNum encrypted = RSA::encrypt(message, e, p * q);
...
TEST(RSA_BigNum_test, encryption_and_decryption_tst_for_string_2)
{
    ...
    std::string encrypted_text = RSA::encrypt(text, e, p * q);
    std::string decrypted_text = RSA::decrypt(encrypted_text, d, p * q);
...
TEST(RSA_BigNum_test, isPrime_test_with_non_prime_number)
{
    ...
    BigNum p(num1);
    BigNum q(num2);
    EXPECT_FALSE(RSA::isPrime(p * 2, 40));
    EXPECT_FALSE(RSA::isPrime(q * 2, 40));
}
